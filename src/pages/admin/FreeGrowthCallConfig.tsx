...
const DEFAULT_DATA = {
    ...
    testimonials: [ // Changed from 'testimonial' to 'testimonials' array
        {
            quote: { en: 'They helped us grow 300% in 3 months.' },
            author: { en: 'Jane Doe' },
            role: { en: 'CEO, TechCorp' },
            clientImage: '' // Added clientImage
        }
    ],
    trustedLogos: [], // Added trustedLogos
    whatToExpect: [
        ...
        { title: { en: 'Actionable Roadmap' }, description: { en: 'Leave with a clear plan.' } }
    ],
    process: [
        ...
        { step: { en: 'Step 3' }, title: { en: 'Strategy Session' }, description: { en: 'Meet with an expert.' } },
        { step: { en: 'Step 4' }, title: { en: 'Execution Roadmap' }, description: { en: 'Get your custom plan.' } }
    ],
    faq: [
...
const AdminFreeGrowthCallConfig = () => {
    ...
    const [activeImageField, setActiveImageField] = useState<{
        type: 'logo' | 'testimonial_image' | 'video_thumbnail' | 'process_image';
        index: number; // -1 for single objects like video_thumbnail
    } | null>(null);
    ...
    const openPicker = (type: 'logo' | 'testimonial_image' | 'video_thumbnail' | 'process_image', index: number) => {
        setActiveImageField({ type, index });
        setPickerOpen(true);
    };
    const [activeTab, setActiveTab] = useState<'hero' | 'booking' | 'expect' | 'process' | 'faq' | 'social' | 'logos'>('hero');
            ...
            <div className="flex flex-col md:flex-row gap-6 h-full">
                ...
                <div className="w-full md:w-64 space-y-2">
                    {[
                        ...
                        { id: 'social', label: 'Success Stories', icon: MessageSquare }, // Changed icon and label
                        { id: 'logos', label: 'Trusted Logos', icon: ShieldCheck }, // New tab
                ...
                <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm overflow-y-auto">
                    ...
                    {activeTab === 'expect' && (
                        <div className="space-y-6">
                            ...
                            >
                                <Plus size={16} /> Add Expectation Item
                            </button>
                        </div>
                    )}
