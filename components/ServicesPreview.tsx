                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-4">
                            <div className={`p-4 rounded-2xl bg-gradient-to-r ${service.color} text-white shadow-lg`}>
                                ...
                                {getIcon(service.icon, "w-8 h-8") || <span className="w-8 h-8"><FaRocket /></span>}
                            </div>
                            <div>
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {service.features?.map((feature, index) => (
                                    <div key={index} className="flex items-center gap-3 bg-slate-50 dark:bg-white/5 p-3 rounded-xl border border-slate-100 dark:border-white/5">
                                        <span className="w-5 h-5 text-blue-600 dark:text-blue-400 flex-shrink-0"><FaCheck /></span>
                        ...
                        <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl text-center border border-slate-100 dark:border-white/5">
                            <div className="flex items-center justify-center gap-4 mb-2">
                                <div className="flex items-center gap-1">
                                    {[...Array(5)].map((_, i) => (
                                        <span key={i} className="w-4 h-4 text-yellow-500 dark:text-yellow-400"><FaStar /></span>
                                    ))}
                                </div>
    ...
    return (
        ...
        >
            ...
            <div className="p-6 md:p-8 flex-1 relative z-10">
                ...
                {/* Visual Content Area - Image + Floating Elements */}
                <div className="mb-6 relative h-40 w-full bg-slate-100 dark:bg-slate-800/50 rounded-xl overflow-hidden border border-slate-200 dark:border-white/5 group-hover:border-blue-500/30 transition-colors">
                    ...
                    {/* Custom Manual Floating Logos - HIGHEST PRIORITY */}
                    {service.floatingLogos && service.floatingLogos.some(l => l) ? (
                        <div className="absolute inset-0 z-20 pointer-events-none">
                            <div className="absolute inset-0 flex items-center justify-center">
                                {/* Logo 1 - Top Right */}
                                {service.floatingLogos[0] && (
                                    <motion.div
                                ...
                                {/* Logo 3 - Top Left / Center Offset */}
                                {service.floatingLogos[2] && (
                                    <motion.div
                        ...
                        <>
                            {/* Social Media Visual Artifact */}
                            {(service.visualType === 'social' || (!service.visualType && isSocialMedia)) && (
                                <div className="absolute inset-0 z-10">
                                    {/* Floating 3D Mockups - Only show Phone if NO image is uploaded */}
                            ...
                            {/* Brand Growth Visual Artifact */}
                            {(service.visualType === 'marketing' || (!service.visualType && isBrandGrowth)) && (
                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                    {/* Central Chart Element - Only show if NO image */}
                                    {!service.image && (
                                        <motion.div
                            ...
                            {/* UI/UX Design Visual Artifact */}
                            {(service.visualType === 'design' || (!service.visualType && isUIUX)) && (
                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                    {/* Abstract UI Elements - Only show if NO image */}
                                    {!service.image && (
                            ...
                            {/* Web Development Visual Artifact */}
                            {(service.visualType === 'development' || (!service.visualType && isWebDev)) && (
                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                    {/* Code Window - Only show if NO image */}
                                    {!service.image && (
                            ...
                            {/* Virtual Assistance Visual Artifact */}
                            {(service.visualType === 'assistant' || (!service.visualType && isVA)) && (
                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                    {/* Central Task Element - Only show if NO image */}
                                    {!service.image && (
                                        <motion.div
                            ...
                            {/* Customer Support Visual Artifact */}
                            {(service.visualType === 'support' || (!service.visualType && isSupport)) && (
                                <div className="absolute inset-0 flex items-center justify-center z-10">
                                    {/* Central Support Element - Only show if NO image */}
                                    {!service.image && (
                                        <motion.div
                                            animate={{ scale: [1, 1.05, 1] }}
                ...
                <div className="space-y-3 mt-auto">
                    ...
                    >
                        <span className="w-4 h-4 text-green-500"><FaWhatsapp /></span>
                        ...
                        <GlassCard className="p-8 md:p-12 max-w-4xl mx-auto bg-gradient-to-br from-slate-900 to-slate-800 dark:from-white/10 dark:to-white/5 border-none text-white">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.95 }}
                                >
                                    <span className="w-5 h-5"><FaRocket /></span>
                                    {t('services.preview.cta_button')}
                                    <span className="w-4 h-4"><FaArrowRight /></span>
                                </motion.button>
                                <motion.button
                                    onClick={() => window.open('https://wa.me/15551234567', '_blank')}
                                    ...
                                    whileTap={{ scale: 0.98 }}
                                >
                                    <span className="w-5 h-5"><FaWhatsapp /></span>
                                    {t('common.chat_whatsapp')}
                                </motion.button>
                            </div>
